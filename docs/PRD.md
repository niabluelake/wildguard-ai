...
## 14. 서비스화 업데이트: 회원 기반 위험도 예측 관리

### 업데이트 목적
기존 1차 ML 기능은 데이터 입력 후 위험도만 출력하는 과제형 구조에 가까웠다. 이를 공공기관형 야생동물 위험 관리 서비스처럼 보이도록 회원가입, 로그인, 사용자별 예측 기록 저장 기능을 추가한다.

### 적용 정책
| 사용자 상태 | 제공 기능 | DB 저장 여부 |
|---|---|---|
| 비회원 | 위험도 예측 체험 | 저장 안 함 |
| 회원 | 위험도 예측, 예측 기록 저장, 내 기록 조회 | 저장함 |
| 지자체 담당자/관리자 | 전체 예측 기록 조회, 통계 대시보드 확장 | 저장함 |

### 추가 화면
| URL | 화면 | 설명 |
|---|---|---|
| `/auth/register` | 회원가입 | 사용자 계정 생성 |
| `/auth/login` | 로그인 | 사용자 인증 및 세션 생성 |
| `/auth/logout` | 로그아웃 | 세션 종료 |
| `/risk` | 위험도 예측 | 비회원은 체험, 회원은 저장 |
| `/risk/history` | 예측 기록 | 로그인 사용자의 예측 기록 조회 |
| `/dashboard` | 통계 대시보드 | 위험도 분포, 최근 기록, 고위험 건수 표시 예정 |
...
| organization | VARCHAR2(100) | 소속 기관/단체 |
| role | VARCHAR2(30) | 사용자 유형 |
| created_at | DATE | 가입일 |

#### RISK_PREDICTION_LOG 변경
기존 예측 로그 테이블에 `user_id` 컬럼을 추가하여 로그인 사용자의 예측 결과를 사용자 기준으로 저장한다.
...
| 항목 | 성공 기준 |
|---|---|
| 회원가입 | 사용자가 계정을 생성할 수 있다. |
| 로그인 | 로그인 성공 시 세션에 사용자 정보가 저장된다. |
| 비회원 체험 | 로그인하지 않아도 위험도 예측 결과를 확인할 수 있다. |
| 회원 저장 | 로그인 상태에서 예측하면 Oracle DB에 저장된다. |
| 기록 조회 | 로그인 사용자가 본인의 예측 기록을 확인할 수 있다. |

### 발표용 설명
WildGuard AI는 비회원에게도 위험도 예측 체험을 제공하지만, 예측 기록 저장과 이력 조회는 회원에게만 제공한다. 이를 통해 단순 ML 예측 화면이 아니라 공공기관과 현장 관리자가 사용할 수 있는 야생동물 출몰 위험 관리 서비스 형태로 확장한다.